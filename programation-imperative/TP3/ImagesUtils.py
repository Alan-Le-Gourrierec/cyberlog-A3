...
#On en a besoin pour "contourner les pointeurs" (de Ewan Palavicini)
def copy(image):
    copie = empty_img(get_width(image), get_height(image))
    for i in range (get_width(image)):
        for j in range (get_height(image)):
            for k in range (3):
