...
public:
    ...
    void printShortestPath(int startNode, int endNode, const vector<int> &distance, const vector<vector<int>> &previousNodes)
    {
        if (distance[endNode] == numeric_limits<int>::max())
        {
            cerr << "No path from Node" << startNode + 1 << " to Node" << endNode + 1 << endl;
            return;
        }

        cout << "Shortest path from Node" << startNode + 1 << " to Node" << endNode + 1 << ": " << distance[endNode] << ", Path: ";
        printPath(startNode, endNode, previousNodes);
        cout << endl;
    }
...
int main()
{
    string PATH = "Examples/test64.csv";
    ifstream in_file;
    ...
    graph.printShortestPath(startNode, endNode, distance, previousNodes);
